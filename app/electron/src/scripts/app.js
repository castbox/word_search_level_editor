...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (autoFillButton) {
    autoFillButton.addEventListener('click', () => {
      ...
      // 自动填充后检测奖励单词
      setTimeout(() => {
        window.gridInstance.detectBonusWords();
      }, 1000); // 等待填充完成后再检测
    });
  }
  ...
  if (generateLevelButton) {
    ...
    generateLevelButton.addEventListener('click', () => {
  ...
  function checkElectronAPI() {
    // 检查electronAPI是否存在及其方法（此API应由预加载脚本提供）
    if (!window.electronAPI) {
      console.error('electronAPI 不存在!');
      alert('electronAPI不存在，应用可能无法保存文件。请检查控制台错误。');
      return false;
    } else {
      console.log('electronAPI 已加载，方法列表:', Object.keys(window.electronAPI).join(', '));
      
      // 检查各个关键方法
      const methods = ['saveConfig', 'saveLevel', 'getSavedLevels', 'deleteLevel', 'readDictionary', 'generateLevel'];
      let allMethodsAvailable = true;
      
      methods.forEach(method => {
        if (typeof window.electronAPI[method] !== 'function') {
          console.error(`electronAPI.${method} 不是函数或不存在!`);
          allMethodsAvailable = false;
        } else {
          console.log(`electronAPI.${method} 可用`);
        }
      });
  ...
  saveButton.addEventListener('click', () => {
    try {
      ...
      const bonusWords = window.gridInstance.detectBonusWords();
  ...
  try {
    ...
    levels.forEach(level => {
      ...
      card.querySelector('.edit-btn').addEventListener('click', async (e) => {
        ...
        try {
          // 修正路径问题 - 使用fsAPI替代fs
          if (level._filePath && window.fsAPI && !window.fsAPI.existsSync(level._filePath)) {
            console.warn('原始文件路径不存在:', level._filePath);
            // 可能是打包后路径变化，尝试从Electron获取正确路径
            const fileName = window.pathAPI.basename(level._filePath);
            showStatusMessage(`正在加载关卡...`, 'info');
            
            try {
              // 重新获取所有关卡来找到正确的路径
              const refreshedLevels = await window.electronAPI.getSavedLevels();
              const matchingLevel = refreshedLevels.find(l => window.pathAPI.basename(l._filePath) === fileName);
              
              if (matchingLevel) {
                console.log('找到替代路径:', matchingLevel._filePath);
                level._filePath = matchingLevel._filePath;
              }
            } catch (pathError) {
              console.error('尝试修正路径时出错:', pathError);
            }
          }
      ...
      card.querySelector('.delete-btn').addEventListener('click', async (e) => {
        ...
        if (confirm(`确定要删除关卡 "${levelTitle}" 吗？此操作不可撤销。`)) {
          try {
            ...
            // 修正路径问题 - 使用fsAPI替代fs
            if (filePath && window.fsAPI && !window.fsAPI.existsSync(filePath)) {
              console.warn('删除关卡: 原始文件路径不存在:', filePath);
              // 可能是打包后路径变化，尝试从Electron获取正确路径
              ...
              try {
                ...
                if (matchingLevel) {
                  console.log('找到替代路径:', matchingLevel._filePath);
                  filePath = matchingLevel._filePath;
                }
              } catch (pathError) {
                console.error('尝试修正路径时出错:', pathError);
              }
...
window.loadLevelIntoEditor = function(levelData) {
  try {
    ...
    if (levelData.difficulty) {
      difficultyInput.value = levelData.difficulty;
      console.log(`加载关卡难度: ${levelData.difficulty}`);
    } else {
