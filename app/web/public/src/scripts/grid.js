class Grid {
  ...
  updateManualCell(row, col, letter) {
    ...
    this.detectBonusWords().catch(error => {
      console.error('自动检测bonus words失败:', error);
    });
  }
  ...
  removeWordFromGrid(wordIndex, triggerChange = true) {
    ...
    if (triggerChange) {
      ...
      setTimeout(async () => {
        try {
          await this.detectBonusWords();
        } catch (error) {
          console.error('检测奖励单词失败:', error);
        }
      }, 100);
    }
  ...
  async detectBonusWords() {
    try {
      ...
      // 2. 字典
      let dictSet = this.dictionarySet || new Set();
  ...
  initRefreshBonusButton() {
    ...
    refreshBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        showStatusMessage('奖励单词检测失败: ' + error.message, 'error');
      } finally {
        // 恢复按钮状态
      ...
      if (filledCount >= emptyPositions.length) {
        ...
        setTimeout(async () => {
          try {
            ...
            console.error('检测bonus words失败:', error);
          }
        }, 800);
